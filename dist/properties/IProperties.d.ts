/// <reference types="react" />
import { IPin } from '../store/IState';
export declare const markerBase64 = "data:img/png;base64,";
export interface IDefaultProps {
    onSaved: (e: ISaveData) => void;
    data: IPin[];
    srcImg: string;
    zoom: number;
}
export interface ISaveData {
    data?: IPin[];
    zoom: number;
}
export interface InputProperties {
    svgRef: React.RefObject<SVGSVGElement>;
}
export declare const defaultProps: IDefaultProps;
export interface ISizeBackground {
    width: number;
    heigth: number;
}
export declare const sizeBackground: ISizeBackground;
export interface ISizePin {
    width: number;
    heigth: number;
}
export declare const sizePin: ISizePin;
//# sourceMappingURL=IProperties.d.ts.map