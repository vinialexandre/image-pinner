...
import { actionEditCoordinatesPin } from '../action/ActionComponent';
import { useDispatch } from '../store/Provider';
import { Dispatch } from 'react';
import { ActionTypes } from '../action/ActionTypes';
type Reference<T> = React.RefObject<T>;

export const createPinDrag = (
  ref: Reference<SVGSVGElement>,
  pins: IPin[],
  newData: IPin,
  save: void
): IPin => {
  ...
  const pinObj = d3
    ...
    .attr(
      'transform',
      `translate(${newData.endX ? newData.endX : 0},${
      newData.endY ? newData.endY : 0
      })`
    )
    .append('image')
  ...
  d3.select(`#${ID}`).call(drag as any);
  return pin;
};

export const updateOrderPinDrag = (
  newData: IPin,
  pins: IPin[]
) => {
...
export function matchOrder(pin: IPin, pins: IPin[]): number {
  if(pins.length){
    pins.sort((a, b) => {
      if (a.order > b.order) {
        return 1;
...
export function repositionTextPin(pin: IPin): string {
  if(pin && pin.order > 9)
    return '12';
  return '17';
}
